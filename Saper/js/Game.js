...
//dzięki extends można używać metod z UI
class Game extends UI {
  //prywatny obiekt, można używać tylko wewnątrz tej klasy
  #config = {
    easy: {
      rows: 8,
      cols: 8,
  ...
  #newGame(
    ...
    this.#modal.infoText =
      this.#timer.numberOfSeconds < this.#timer.maxNumberOfSeconds
        ? `You won, it took you ${this.#timer.numberOfSeconds}, congratulations`
  ...
  #handleElements() {
    //pobranie z DOM article, gdzie będzie umieszczana tablica (metoda z UI.js)
  ...
  #renderBoard() {
    ...
    //z tablicy dwuwymiarowej => jednowymiarowa
  ...
  #placeMinesInCells() {
    ...
    while (minesToPlace) {
      ...
      //sprawdzenie, czy w wylosowanej komórce jest już mina
  ...
  #setCellValue(cell) {
    let minesCount = 0;
    //Math.max wybiera największą liczbę z przedziału
    for (
    ...
    // automatyczne odkrycie okoliczny pól jeśli komórka nie ma min wokół siebie
    if (!cell.value) {
      for (
        let rowIndex = Math.max(cell.y - 1, 0);
        rowIndex <= Math.min(cell.y + 1, this.#numberOfRows - 1);
